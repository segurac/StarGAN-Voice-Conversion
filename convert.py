...
class SpeakerStats(object):
    def __init__(self, config, src_spk):
        self.src_spk = src_spk
        self.spk_idx = spk2idx[src_spk]
        spk_cat = to_categorical([self.spk_idx], num_classes=len(speakers))
        self.spk_c_src = spk_cat

        self.mc_files = sorted(glob.glob(join(config.test_data_dir, f'{src_spk}*.npy')))
        self.src_spk_stats = np.load(join(config.train_data_dir, f'{src_spk}_stats.npz'))
        self.src_wav_dir = f'{config.wav_dir}/{src_spk}'
...
##class TestDataset(object):
    ...
    ##def __init__(self, config, src_spk, trg_spk):
        ...
        ##self.trg_spk_stats = np.load(join(config.train_data_dir, f'{trg_spk}_stats.npz'))
...
def test(config):
    ...
    spkstats = {}
    for spk in speakers:
        spkstats[spk] = SpeakerStats(config, spk)
        
    #Read conversion list file
...
if __name__ == '__main__':
    ...
    parser.add_argument('--convert_list', type=str, default='../list_seed0_test.tsv')
